...
/**
 * Servlet implementation class GetBookByTitleServlet
 */
@WebServlet("/findByTitle.search")
public class GetBookByTitleServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(title == null){ gson.toJson("error",response.getWriter()); return;}
		
		AdminDQLService ads = new AdminDQLService();
		
		ArrayList<BookStocks> bookArrays = ads.getBookStocksArray(title);
		...
		{
			gson.toJson(bookArrays,response.getWriter());
		}
